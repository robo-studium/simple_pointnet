...
class WADSDataset(Dataset):
    """WADS Dataset for weather noise removal"""
    ...
    def __init__(
        ...
        fov_up=2.0,
        fov_down=-24.9,
        noise_label=110,  # Label value for noise points
    ):
        """
        Args:
            root_dir: Path to WADS directory
            sequences: List of sequence numbers to include
            ...
            noise_label: Label value that indicates noise
        """
...
def create_dataloaders(
    ...
    train_dataset = WADSDataset(
        root_dir, splits["train"], proj_H, proj_W, noise_label=noise_label
    )
    val_dataset = WADSDataset(
        root_dir, splits["val"], proj_H, proj_W, noise_label=noise_label
    )
    test_dataset = WADSDataset(
        root_dir, splits["test"], proj_H, proj_W, noise_label=noise_label
    )
